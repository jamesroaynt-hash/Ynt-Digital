...
| GET | `/billing` | Per visible ad account: payment method, balance, spend cap, daily spend limit and account standing read live from Meta (never stored), plus today / month-to-date / range spend from the synced insights. It also returns `charges` — Meta's own payment activity per transaction (id, date, amount, tax, payment method, `tracking_id` card reference, `vat_invoice_id`, status, ad-credit flag) for every visible account, capped at eight accounts per request — and `charges_by_month` (what the card was actually charged: refunds netted off, declines and pending kept apart, and ad credit counted separately because it never touches the card). Meta gates the charge edge on some accounts — that comes back as `charges_note`, not an error. |
