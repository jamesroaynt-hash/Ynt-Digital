...
function runMigrations(db) {
  ...
  `);
  // InfoTXT SMS automation: a custom list of tag → message rules. When a POS
  // order gains a tag matching an enabled rule, the customer is texted once.
  db.exec(`
    CREATE TABLE IF NOT EXISTS sms_tag_rules (
  ...
  `);
  // One row per (order, tag) that was texted — dedup so a tag never re-sends.
  db.exec(`
...
async function runPostgresMigrations(db) {
  ...
  await db.exec(`
    ...
    ON CONFLICT(shop_id, external_id) DO NOTHING
  `);
  // InfoTXT SMS automation — see runMigrations() for rationale.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS sms_tag_rules (
      tag TEXT PRIMARY KEY,
      message TEXT NOT NULL DEFAULT '',
