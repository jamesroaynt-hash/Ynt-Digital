...
function runMigrations(db) {
  ...
  `);
  try { db.exec('ALTER TABLE google_orders DROP COLUMN external_id'); } catch {}
  db.exec('DROP INDEX IF EXISTS idx_google_orders_tracking');
  db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_google_orders_tracking_unique ON google_orders(tracking_no)');
...
async function runPostgresMigrations(db) {
  ...
  `);
  try { await db.exec('ALTER TABLE google_orders DROP COLUMN IF EXISTS external_id'); } catch {}
  await db.exec('DROP INDEX IF EXISTS idx_google_orders_tracking');
  await db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_google_orders_tracking_unique ON google_orders(tracking_no)');
