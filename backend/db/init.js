...
// sms_rules shipped rider-only, with the trigger unique per provider. Customer
// messages reuse the same triggers ("on delivery" texts both), so the recipient
// has to join that key — which on SQLite means rebuilding the table, since the
// UNIQUE is baked into the CREATE. Existing rules are all rider rules.
function migrateSmsRulesRecipient(db) {
  const columns = db.prepare('PRAGMA table_info(sms_rules)').all();
  if (columns.some((c) => c.name === 'recipient')) return;

  db.exec('PRAGMA foreign_keys = OFF');
  db.exec('ALTER TABLE sms_rules RENAME TO _sms_rules_rider_only_tmp');
  db.exec(`
  ...
  `);
  db.exec(`
    INSERT INTO sms_rules (id, provider, recipient, trigger_type, trigger_value, message, enabled, created_at, updated_at)
    SELECT id, provider, 'rider', trigger_type, trigger_value, message, enabled, created_at, updated_at
    FROM _sms_rules_rider_only_tmp
  `);
  db.exec('DROP TABLE _sms_rules_rider_only_tmp');
  db.exec('PRAGMA foreign_keys = ON');
}

async function migrateSmsRulesRecipientAsync(db) {
  if (db.type !== 'postgres') {
    migrateSmsRulesRecipient(db);
    return;
  }
  await ensureColumnAsync(db, 'sms_rules', 'recipient', "TEXT NOT NULL DEFAULT 'rider'");
  try {
    await db.exec('ALTER TABLE sms_rules DROP CONSTRAINT IF EXISTS sms_rules_provider_trigger_type_trigger_value_key');
    await db.exec(`
      CREATE UNIQUE INDEX IF NOT EXISTS sms_rules_provider_recipient_trigger_key
      ON sms_rules (provider, recipient, trigger_type, trigger_value)
    `);
  } catch { /* Supabase migration handles this */ }
}
...
function runMigrations(db) {
  ...
  // star rating. The star columns stay behind on existing databases so the
  // rows already filed under the old form are not thrown away.
  ensureColumn(db, 'evaluations', 'items', 'TEXT');
  ensureColumn(db, 'evaluations', 'development_areas', 'TEXT');
  ensureColumn(db, 'evaluations', 'proceed_to_final', 'TEXT');
  ensureColumn(db, 'evaluations', 'passed', 'TEXT');
  ensureColumn(db, 'evaluation_weights', 'communication_weight', 'REAL NOT NULL DEFAULT 25');
  ensureColumn(db, 'evaluation_weights', 'attitude_weight', 'REAL NOT NULL DEFAULT 25');
  ensureColumn(db, 'evaluation_weights', 'skills_weight', 'REAL NOT NULL DEFAULT 25');
  ensureColumn(db, 'evaluation_weights', 'technical_weight', 'REAL NOT NULL DEFAULT 25');
  // The score an evaluation has to reach to count as passed, set by HR.
  ensureColumn(db, 'evaluation_weights', 'passing_score', 'REAL NOT NULL DEFAULT 75');
  // The matrix's item rows. Seeded from the printed sheet the first time the
  // evaluations route reads them, and edited from the page after that.
  db.exec(`
  ...
  `);
  db.exec('CREATE INDEX IF NOT EXISTS idx_evaluation_items_criterion ON evaluation_items(criterion_id, position)');
  db.exec(`
  ...
  `);
  // sms_settings shipped with statuses only; tag triggers came later.
  ensureColumn(db, 'sms_settings', 'rider_tags', 'TEXT');
  db.exec(`
  ...
  `);
  db.exec('CREATE INDEX IF NOT EXISTS idx_user_rate_history_lookup ON user_rate_history(user_id, effective_from)');
  // Backfill: seed each existing user's current rate as effective from the epoch
  // so every historical date resolves to today's rate — no retroactive change on
  // rollout. Future rate changes insert dated rows that only affect dates onward.
  db.exec(`
    INSERT INTO user_rate_history (user_id, daily_rate, effective_from)
    SELECT id, COALESCE(daily_rate, 0), '1970-01-01'
    FROM users u
    WHERE NOT EXISTS (SELECT 1 FROM user_rate_history h WHERE h.user_id = u.id)
  `);
  db.exec(`
  ...
  `);
  db.exec('CREATE INDEX IF NOT EXISTS idx_chat_messages_id ON chat_messages(id DESC)');
  db.exec(`
  ...
  // Daily CSR Performance Report: one hand-entered card per CSR per day, plus
  // the team's per-product counts (pending new / awaiting print / confirms).
  db.exec(`
    CREATE TABLE IF NOT EXISTS csr_daily_reports (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      report_date TEXT NOT NULL,
      user_id INTEGER NOT NULL REFERENCES users(id),
      shift TEXT NOT NULL DEFAULT 'AM',
      upsell_confirm INTEGER NOT NULL DEFAULT 0,
      new_confirm INTEGER NOT NULL DEFAULT 0,
      broadcast INTEGER NOT NULL DEFAULT 0,
      pending INTEGER NOT NULL DEFAULT 0,
      cancelled INTEGER NOT NULL DEFAULT 0,
      upsell_pending_confirm INTEGER NOT NULL DEFAULT 0,
      pending_confirm INTEGER NOT NULL DEFAULT 0,
      pending_out INTEGER NOT NULL DEFAULT 0,
      awaiting_print INTEGER NOT NULL DEFAULT 0,
      pending_cancelled INTEGER NOT NULL DEFAULT 0,
      updated_by INTEGER REFERENCES users(id),
      updated_at TEXT NOT NULL DEFAULT (datetime('now')),
      UNIQUE(report_date, user_id)
    )
  `);
  // The product rows the Daily CSR Report lists, in order. Shared by every day;
  // empty until someone edits the list, when the built-in defaults are used.
  db.exec(`
    CREATE TABLE IF NOT EXISTS csr_daily_products (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  `);
  db.exec(`
    CREATE TABLE IF NOT EXISTS csr_daily_team (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      report_date TEXT NOT NULL,
      ...
      updated_by INTEGER REFERENCES users(id),
      updated_at TEXT NOT NULL DEFAULT (datetime('now')),
      UNIQUE(report_date, section, product)
    )
  `);
  // ODZ Finder: reference list of areas flagged as an ODZ (out of delivery
  // zone) or a Settlement pick-up area, keyed by Province / City / Barangay.
  db.exec(`
    CREATE TABLE IF NOT EXISTS odz_areas (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      province TEXT NOT NULL DEFAULT '',
      city TEXT NOT NULL DEFAULT '',
      brgy TEXT NOT NULL DEFAULT '',
      status TEXT NOT NULL DEFAULT 'ODZ',
      created_by INTEGER REFERENCES users(id),
      created_at TEXT NOT NULL DEFAULT (datetime('now')),
      updated_at TEXT NOT NULL DEFAULT (datetime('now'))
    )
  `);
  db.exec('CREATE INDEX IF NOT EXISTS idx_odz_areas_loc ON odz_areas(province, city, brgy)');
...
async function runPostgresMigrations(db) {
  ...
  // star rating. The star columns stay behind on existing databases so the
  // rows already filed under the old form are not thrown away.
  await ensureColumnAsync(db, 'evaluations', 'items', 'TEXT');
  await ensureColumnAsync(db, 'evaluations', 'development_areas', 'TEXT');
  await ensureColumnAsync(db, 'evaluations', 'proceed_to_final', 'TEXT');
  await ensureColumnAsync(db, 'evaluations', 'passed', 'TEXT');
  await ensureColumnAsync(db, 'evaluation_weights', 'communication_weight', 'NUMERIC NOT NULL DEFAULT 25');
  await ensureColumnAsync(db, 'evaluation_weights', 'attitude_weight', 'NUMERIC NOT NULL DEFAULT 25');
  await ensureColumnAsync(db, 'evaluation_weights', 'skills_weight', 'NUMERIC NOT NULL DEFAULT 25');
  await ensureColumnAsync(db, 'evaluation_weights', 'technical_weight', 'NUMERIC NOT NULL DEFAULT 25');
  // The score an evaluation has to reach to count as passed, set by HR.
  await ensureColumnAsync(db, 'evaluation_weights', 'passing_score', 'NUMERIC NOT NULL DEFAULT 75');
  // The matrix's item rows. Seeded from the printed sheet the first time the
  // evaluations route reads them, and edited from the page after that.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS evaluation_items (
  ...
  `);
  await db.exec('CREATE INDEX IF NOT EXISTS idx_evaluation_items_criterion ON evaluation_items(criterion_id, position)');
  await db.exec(`
    CREATE TABLE IF NOT EXISTS expense_credits (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      credit_ref TEXT NOT NULL UNIQUE,
  ...
  `);
  // sms_settings shipped with statuses only; tag triggers came later.
  await ensureColumnAsync(db, 'sms_settings', 'rider_tags', 'TEXT');
  await db.exec(`
    CREATE TABLE IF NOT EXISTS sms_rules (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      provider TEXT NOT NULL,
  ...
  `);
  await migrateSmsRulesRecipientAsync(db);
  await db.exec(`
    CREATE TABLE IF NOT EXISTS overtime_requests (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      user_id INTEGER NOT NULL REFERENCES users(id),
  ...
  `);
  await db.exec('CREATE INDEX IF NOT EXISTS idx_user_rate_history_lookup ON user_rate_history(user_id, effective_from)');
  // Backfill: seed each existing user's current rate as effective from the epoch
  // so every historical date resolves to today's rate — no retroactive change on
  // rollout. Future rate changes insert dated rows that only affect dates onward.
  await db.exec(`
  ...
  `);
  await db.exec('CREATE INDEX IF NOT EXISTS idx_chat_messages_id ON chat_messages(id DESC)');
  await db.exec(`
    CREATE TABLE IF NOT EXISTS webhook_subscriptions (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      name TEXT NOT NULL,
  ...
  `);
  await db.exec('CREATE INDEX IF NOT EXISTS idx_odz_areas_loc ON odz_areas(province, city, brgy)');
  // Append-only per-customer notes (keyed by normalized phone). Independent of
  // pos_orders so the history survives the 30-day order retention.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS customer_notes (
