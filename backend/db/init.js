...
function runMigrations(db) {
  ...
  // "Pick Up" (any tag containing "pick up"), stamped with the Manila day.
  ...
  // Idempotent backfill: seed any orders currently carrying a "pick up" tag so
  ...
  db.exec(`
    ...
    FROM pos_orders
    WHERE LOWER(COALESCE(tags_json,'')) LIKE '%pick up%' AND updated_at_remote IS NOT NULL
...
async function runPostgresMigrations(db) {
  ...
  // the first time an order is tagged "Pick Up" (any tag containing "pick up").
  await db.exec(`
    CREATE TABLE IF NOT EXISTS pickup_log (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      shop_id TEXT,
